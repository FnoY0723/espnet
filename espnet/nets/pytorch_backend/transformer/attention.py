...
class LookaheadMultiHeadedAttention(MultiHeadedAttention):

    def __init__(self, n_head, n_feat, dropout_rate):
        """Construct an RelPositionMultiHeadedAttention object."""
        super().__init__(n_head, n_feat, dropout_rate)
      
    def lookahead_forward_attention(self, value, scores, mask):
        """Compute attention context vector.
        ...
        length = value.size(-2)
        att_mask = torch.tril(torch.ones(length,length), diagonal=2).bool().to(value.device)
        min_value = torch.finfo(scores.dtype).min
    ...
    def forward(self, query, key, value, mask):
        ...
        q, k, v = self.forward_qkv(query, key, value)
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_k)
        return self.lookahead_forward_attention(v, scores, mask)
